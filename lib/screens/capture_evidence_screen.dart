...
class CaptureEvidenceScreen extends StatefulWidget {
  const CaptureEvidenceScreen({Key? key}) : super(key: key);
...
class _CaptureEvidenceScreenState extends State<CaptureEvidenceScreen> {
            ...
            children: [
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  ...
                  focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Color(0xFF0A4DDE), width: 2)),
                ),
              ),
              const SizedBox(height: 16),
              const Text(
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              Row(
                children: [
                  // Bar 1 — complete, tap to go back
                  _stepBar(context, true, onTap: () => Navigator.pop(context)),
                  const SizedBox(width: 4),
                  // Bar 2 — current, no tap
                  _stepBar(context, true, onTap: null),
                  const SizedBox(width: 4),
                  // Bar 3 — next, tap to continue (same as Continue button)
                  _stepBar(context, false, onTap: () {
                    if (_imageFile == null && _audioPath == null &&
                        _descriptionController.text.trim().isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                          content: Text('Please capture a photo, audio, or write a description.')));
                      return;
                    }
                    Navigator.pushNamed(context, '/report-incident', arguments: {
                      'docId': _editDocId,
  ...
  Widget _stepBar(BuildContext context, bool filled, {VoidCallback? onTap}) {
    return Expanded(
  ...
  Widget _buildPhotoUploadContainer() {
    ...
    if (_imageFile != null) {
      photoWidget = ClipRRect(
        ...
        child: kIsWeb 
            ? Image.network(_imageFile!.path, fit: BoxFit.cover, height: 260, width: double.infinity)
            : Image.file(File(_imageFile!.path), fit: BoxFit.cover, height: 260, width: double.infinity),
      );
      ...
      photoWidget = Stack(
        ...
        children: [
          ClipRRect(
            borderRadius: BorderRadius.circular(16),
            child: Image.network(_existingPhotoUrl!, fit: BoxFit.cover, height: 260, width: double.infinity),
          ),
          Container(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        padding: (_imageFile == null && _existingPhotoUrl == null) ? const EdgeInsets.symmetric(vertical: 72) : EdgeInsets.zero,
        decoration: BoxDecoration(
  ...
  Widget _buildVoiceRecordContainer() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: _isRecording ? const Color(0xFFEF4444) : const Color(0xFFE5E7EB)),
        boxShadow: [
          BoxShadow(
      ...
      child: Column(
        children: [
          Row(
            children: [
              GestureDetector(
                onTap: (_audioPath != null || _existingAudioUrl != null) ? _togglePlayback : _toggleRecording,
                child: Container(
                  padding: const EdgeInsets.all(12),
                  decoration: BoxDecoration(
                    color: (_audioPath != null || _existingAudioUrl != null) ? const Color(0xFF10B981) : const Color(0xFFEF4444),
                    shape: BoxShape.circle,
                  ),
                  child: Icon(
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        if (_isRecording)
                          ...
                          Text(
                            _existingAudioUrl != null && _audioPath == null && _playbackDuration == Duration.zero
                                ? 'Ready to play'
          ...
          children: [
            ...
            Expanded(
              ...
              child: ElevatedButton(
                onPressed: () {
                  if (_imageFile == null && _audioPath == null && _descriptionController.text.trim().isEmpty) {
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please capture a photo, audio, or write a description.')));
                    return;
                  }
                  
                  // Navigate to step 3 and pass arguments
                  Navigator.pushNamed(context, '/report-incident', arguments: {
                    'docId': _editDocId,
                    'imageFile': _imageFile,
                    'audioPath': _audioPath,
